...
export default function CESARPage() {
  ...
  async function load() { setData(await fetch('/api/cesar').then(r => r.json())) }
  useEffect(() => { load() }, [])
  function showToast(msg: string) { setToast(msg); setTimeout(() => setToast(''), 3000) }

  async function save() {
    if (!form.produto) return alert('Produto obrigatório!')
    const url = form.id ? `/api/cesar/${form.id}` : '/api/cesar'
  ...
  async function del(id: string) {
    ...
    await fetch(`/api/cesar/${id}`, { method: 'DELETE' })
    showToast('Excluído!'); load()
  }

  async function mover() {
    const d = await fetch('/api/cesar/mover', { method: 'POST' }).then(r => r.json())
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-gray-900">CESAR</h1>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setModal(false)}>
          <div className="modal">
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-base font-bold">{form.id ? 'Editar import' : 'Novo import CESAR'}</h2>
